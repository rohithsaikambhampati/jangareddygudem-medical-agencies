...
function EditProductModal({ product, onClose, onSave }) {
 ...
 const inputCls = 'w-full px-3.5 py-2.5 rounded-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/25 focus:border-teal-500 transition text-sm text-slate-900';
 ...
 className="relative bg-white rounded-md shadow-sm w-full max-w-lg overflow-hidden"
 >

 {/* Header */}
 <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 bg-gradient-to-r from-teal-600 to-teal-700">
 <div className="flex items-center gap-3">
 <div className="w-9 h-9 bg-white rounded-md flex items-center justify-center">
 <Pencil className="h-4.5 w-4.5 text-slate-900" />
 </div>
 <div>
 ...
 <button onClick={onClose} className="text-white/70 hover:text-white p-1.5 rounded-md hover:bg-teal-800 transition">
 <X className="h-5 w-5" />
 </button>
 </div>
 ...
 {brands.map(b => <option className="bg-white text-slate-900" key={b} value={b}>{b}</option>)}
 </select>
 </div>
 <div>
 <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Category</label>
 <select name="category" value={form.category} onChange={handleChange} className={`${inputCls} bg-white`}>
 {categories.map(c => <option className="bg-white text-slate-900" key={c} value={c}>{c}</option>)}
 </select>
 </div>
 </div>
 ...
 >
 <div>
 <p className={`text-sm font-bold ${form.isOfferActive ? 'text-teal-800' : 'text-slate-500'}`}>
 ...
 className="flex-1 py-2.5 rounded-md border border-slate-200 text-slate-500 font-semibold text-sm hover:bg-transparent transition">
 Cancel
 </button>
 <button type="submit"
...
export default function OwnerPage() {
 ...
 const inputCls = 'w-full px-4 py-2.5 rounded-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition text-sm';

 const statusFlow = ['processing', 'confirmed', 'delivered'];
 const statusStyles = {
 processing: 'bg-[#f4f4f5] text-slate-500 border-slate-200',
 ...
 >
 {/* Header */}
 <div className="bg-white p-4 rounded-md shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
 <div>
 <h1 className="text-3xl font-bold text-slate-900 tracking-normal flex items-center gap-2">
 ...
 <Package className="h-5 w-5 text-teal-600" /> Order Fulfillment Center
 </h2>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div 
 onClick={() => { setActiveQueueTab('processing'); setView('orders_queue'); }}
 className="bg-white p-4 rounded-md border border-slate-200 shadow-sm flex items-center justify-between group cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(0,0,0,0.3)] hover:border-slate-200 hover:border-amber-200 transition"
 >
 ...
 <div>
 <p className="text-slate-500 text-xs font-bold uppercase tracking-wider">To Be Confirmed</p>
 ...
 <ArrowRight className="h-6 w-6 text-slate-500 group-hover:text-amber-500 group-hover:translate-x-1.5 transition-all" />
 </div>

 <div 
 onClick={() => { setActiveQueueTab('confirmed'); setView('orders_queue'); }}
 className="bg-white p-4 rounded-md border border-slate-200 shadow-sm flex items-center justify-between group cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(0,0,0,0.3)] hover:border-slate-200 hover:border-teal-200 transition"
 >
 <div className="flex items-center gap-4">
 <div className="p-4 bg-teal-50 text-teal-600 rounded-md group-hover:scale-105 transition-transform duration-300">
 <Truck className="h-8 w-8" />
 </div>
 <div>
 <p className="text-slate-500 text-xs font-bold uppercase tracking-wider">To Be Delivered</p>
 <p className="text-3xl font-bold text-slate-850 mt-1">{orders.filter(o => o.status === 'confirmed').length} Orders</p>
 <p className="text-xs text-teal-600 mt-1 font-medium">Click to ship or mark orders as delivered →</p>
 </div>
 </div>
 <ArrowRight className="h-6 w-6 text-slate-500 group-hover:text-teal-500 group-hover:translate-x-1.5 transition-all" />
 </div>
 </div>
 </div>
 ...
 <div>
 <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2 mb-4">
 <Tag className="h-5 w-5 text-teal-600" /> Manage Brands
 </h2>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
 <div className="bg-white p-4 rounded-md border-2 border-dashed border-slate-200 shadow-sm flex flex-col justify-center">
 <h3 className="font-bold text-slate-900 flex items-center gap-2 mb-4">
 <Plus className="text-teal-500" /> Add New Brand
 </h3>
 ...
 className="w-full px-4 py-2.5 rounded-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition text-sm"
 />
 <button
 type="submit"
 disabled={!newBrandName.trim()}
 className="bg-teal-600 text-white font-bold px-5 py-2.5 rounded-md hover:bg-teal-700 transition disabled:opacity-50"
 >
 Add Brand
 </button>
 </form>
 ...
 className="bg-white p-4 rounded-md border border-slate-200 shadow-sm flex flex-col justify-between group cursor-pointer hover:shadow-md hover:border-teal-200 transition"
 >
 <div>
 <div className="flex justify-between items-start mb-4">
 <div className="w-12 h-12 bg-teal-50 text-teal-600 rounded-md flex items-center justify-center font-bold text-xl">
 {b.charAt(0)}
 </div>
 {editingBrandName !== b && (
 <button
 ...
 className="text-slate-500 hover:text-teal-600 p-1.5 rounded-md hover:bg-teal-50 opacity-0 group-hover:opacity-100 transition"
 ...
 className="w-full px-3 py-1.5 rounded-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition text-sm font-semibold"
 ...
 className="bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold px-3 py-1.5 rounded-md transition"
 >
 Save
 </button>
 <button
 onClick={() => setEditingBrandName(null)}
 className="bg-[#f4f4f5] hover:bg-slate-200 text-slate-500 text-xs font-semibold px-3 py-1.5 rounded-md transition"
 >
 Cancel
 </button>
 </div>
 ...
 <p className="font-bold text-slate-900 text-xl group-hover:text-teal-700 transition">{b}</p>
 )}
 </div>
 ...
 className="bg-white p-4 rounded-md border border-slate-200 shadow-sm group cursor-pointer hover:shadow-md hover:border-blue-200 transition"
 >
 ...
 <div>
 <p className="font-bold text-slate-900 leading-tight group-hover:text-blue-700 transition">{user.name}</p>
 <p className="text-xs text-slate-500">@{user.username}</p>
 </div>
 </div>
 <div className="flex justify-between items-center text-sm font-semibold">
 <span className="text-slate-500 bg-transparent px-2 py-1 rounded-md border border-slate-200">{userOrders.length} Orders</span>
 ...
 <div className="col-span-full py-8 text-center border border-dashed border-slate-200 rounded-md bg-white text-slate-500">
 ...
 <div className="bg-white p-4 rounded-md shadow-sm border border-slate-200 flex justify-between items-center gap-4">
 <div>
 <h1 className="text-3xl font-bold text-slate-900 tracking-normal flex items-center gap-3">
 <span className="bg-teal-100 text-teal-700 w-10 h-10 rounded-full flex items-center justify-center text-xl">{selectedBrand.charAt(0)}</span>
 {selectedBrand} Inventory
 </h1>
 </div>
 <button
 onClick={() => setView('hub')}
 className="bg-[#f4f4f5] hover:bg-slate-200 text-slate-900 font-bold py-2.5 px-5 rounded-md flex items-center gap-2 transition"
 >
 ...
 <div className="bg-gradient-to-br from-teal-50 to-teal-100 p-4 rounded-md border border-teal-200/50 shadow-sm relative overflow-hidden">
 <p className="text-teal-800 text-sm font-semibold uppercase tracking-wider">Active Offers</p>
 <h3 className="text-4xl font-bold text-teal-900 mt-2">{brandActiveOffers}</h3>
 </div>
 ...
 <div className="bg-white p-4 rounded-md border border-slate-200 shadow-sm space-y-5 h-fit">
 ...
 <div>
 <label className="block text-sm font-medium text-slate-500 mb-1">Offer (e.g. 10+2)</label>
 ...
 <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div>
 <label className="block text-sm font-medium text-slate-500 mb-1">Batch Number</label>
 ...
 className="w-4 h-4 text-teal-600 border-zinc-400 rounded focus:ring-teal-500" />
 <label htmlFor="isOfferActive" className="text-sm font-medium text-slate-500 cursor-pointer">Activate Offer / Discount Now</label>
 </div>
 {formError && (
 <div className="text-sm text-rose-500 bg-rose-50 p-3 rounded-md border border-rose-100">{formError}</div>
 )}
 <button type="submit" className="w-full bg-teal-600 hover:bg-teal-700 text-white font-semibold py-3 px-4 rounded-md transition shadow-sm flex justify-center items-center gap-2">
 <Plus className="h-5 w-5" /> Add to Catalog
 </button>
 </form>
 </div>

 <div className="bg-white p-4 rounded-md border border-slate-200 shadow-sm lg:col-span-2 space-y-4">
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
 <h2 className="text-xl font-bold text-slate-900">{selectedBrand} Products</h2>
 ...
 className="w-full pl-9 pr-4 py-2 rounded-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition text-sm text-slate-900 bg-white"
 />
 </div>
 </div>
 <div className="overflow-x-auto rounded-md border border-slate-200">
 <table className="min-w-full divide-y divide-slate-100 text-left text-sm">
 <thead className="bg-transparent text-slate-500 font-semibold text-xs uppercase tracking-wider">
 <tr>
 ...
 return (
 <tr key={product.id} onClick={() => setEditingProduct(product)} className="hover:bg-teal-50 transition cursor-pointer">
 ...
 <td className="px-5 py-4 font-semibold text-slate-900">{product.stockQuantity}</td>
 ...
 <td className="px-5 py-4 text-center">
 {product.expiryDate ? <span className="text-xs text-slate-500">{new Date(product.expiryDate).toLocaleDateString()}</span> : <span className="text-xs text-slate-500">N/A</span>}
 </td>
 ...
 >
 {/* Header */}
 <div className="bg-white p-4 rounded-md shadow-sm border border-slate-200 flex justify-between items-center gap-4 flex-wrap">
 <div className="flex items-center gap-4">
 <div className="w-14 h-14 bg-teal-50 text-teal-600 rounded-md flex items-center justify-center border-4 border-white shadow-sm">
 <ShoppingBag className="h-7 w-7" />
 </div>
 <div>
 <h1 className="text-3xl font-bold text-slate-900 tracking-normal">Fulfillment Queue</h1>
 <p className="text-slate-500 font-semibold mt-0.5">Manage and track orders across all retailers</p>
 </div>
 </div>
 <button
 onClick={() => setView('hub')}
 ...
 activeQueueTab === 'processing' ? 'bg-amber-100 text-amber-700' : 'bg-[#f4f4f5] text-slate-500'
 }`}>
 ...
 Pending Delivery
 <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
 activeQueueTab === 'confirmed' ? 'bg-teal-100 text-teal-700' : 'bg-[#f4f4f5] text-slate-500'
 }`}>
 ...
 {/* Orders List */}
 <div className="bg-white p-4 rounded-md border border-slate-200 shadow-sm space-y-5">
 {filteredOrders.length === 0 ? (
 <div className="text-center py-16 text-slate-500 border border-dashed border-slate-200 rounded-md">
 ...
 return (
 <div key={order.id} className="p-5 bg-transparent border border-slate-200 rounded-md space-y-4 hover:bg-[#f4f4f5] transition">
 ...
 <p className="font-bold text-slate-900 mt-2 text-base">{order.productName}</p>
 ...
 {/* Order details */}
 <div className="grid grid-cols-3 gap-2 bg-[#f4f4f5]/40 p-3 rounded-md border border-slate-200 text-center">
 <div className="p-1">
 <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Billed</p>
 <p className="text-sm sm:text-base font-bold text-slate-900 mt-0.5">{order.quantity}</p>
 </div>
 <div className="p-1">
 <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Dispatched</p>
 <p className="text-sm sm:text-base font-bold text-slate-900 mt-0.5">{order.totalDispatched || order.quantity}</p>
 </div>
 <div className="p-1">
 ...
 <p className="text-base sm:text-lg font-bold text-teal-700 font-mono mt-0.5">₹{order.totalPrice?.toFixed(2)}</p>
 </div>
 {order.status !== 'processing' && order.status !== 'cancelled' && (
 <Link to={`/invoice/${order.id}`} 
 className="bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold px-3 py-2 rounded-md flex items-center gap-1.5 transition shadow-sm">
 ...
 <div className="bg-white border border-slate-200 rounded-md p-3 flex gap-3">
 <MapPin className="h-4 w-4 text-slate-500 shrink-0 mt-0.5" />
 <div>
 <p className="text-xs font-bold text-slate-900">{order.deliveryAddress?.name} ({order.deliveryAddress?.phone})</p>
 <p className="text-xs text-slate-500 mt-0.5">
 ...
 animate={{ opacity: 1, scale: 1, y: 0 }}
 exit={{ opacity: 0, scale: 0.95, y: 20 }}
 className="relative bg-white rounded-md shadow-sm w-full max-w-md overflow-hidden"
 >
 <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 bg-gradient-to-r from-teal-600 to-teal-700 text-white">
 <div>
 ...
 <button onClick={() => setShowPaymentModal(false)} className="text-white/70 hover:text-white p-1.5 rounded-md hover:bg-teal-800 transition">
 <X className="h-5 w-5" />
 </button>
 </div>
 ...
 <div className="flex gap-3 pt-2">
 <button type="button" onClick={() => setShowPaymentModal(false)} className="flex-1 py-2.5 rounded-md border border-slate-200 text-slate-500 font-semibold text-sm hover:bg-transparent transition">
 Cancel
 </button>
 <button type="submit" className="flex-1 py-2.5 rounded-md bg-teal-600 hover:bg-teal-700 text-white font-bold text-sm shadow-sm transition">
 Record Payment
 </button>
 </div>
 </form>
 ...
 <div className="bg-white p-4 rounded-md shadow-sm border border-slate-200 flex justify-between items-center gap-4 flex-wrap">
 <div className="flex items-center gap-4">
 <div className="w-14 h-14 bg-blue-50 text-blue-600 rounded-md flex items-center justify-center border-4 border-white shadow-sm">
 <User className="h-7 w-7" />
 </div>
 <div>
 <h1 className="text-3xl font-bold text-slate-900 tracking-normal">{selectedRetailer?.name || 'Retailer'}</h1>
 ...
 className="bg-rose-100 hover:bg-rose-200 text-rose-600 font-bold py-2.5 px-4 rounded-md flex items-center gap-2 transition"
 ...
 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
 <div className="bg-white p-4 rounded-md border border-rose-100 shadow-sm flex items-center gap-5">
 ...
 <p className="text-4xl font-bold text-slate-900 mt-1">₹{pendingAmount.toFixed(2)}</p>
 <p className="text-xs text-slate-500 mt-1">Outstanding outstanding amount</p>
 </div>
 </div>
 <div className="bg-white p-4 rounded-md border border-teal-100 shadow-sm flex items-center gap-5">
 <div className="p-4 bg-teal-50 text-teal-500 rounded-md"><IndianRupee className="h-8 w-8" /></div>
 <div>
 <p className="text-teal-600 text-sm font-bold uppercase tracking-wider">Total Amount Paid</p>
 <p className="text-4xl font-bold text-slate-900 mt-1">₹{totalPaid.toFixed(2)}</p>
 <p className="text-xs text-slate-500 mt-1">From {retailerPayments.length} logged payments</p>
 </div>
 </div>
 <div className="bg-white p-4 rounded-md border border-emerald-100 shadow-sm flex items-center gap-5">
 ...
 <p className="text-4xl font-bold text-slate-900 mt-1">₹{totalLifetime.toFixed(2)}</p>
 ...
 <div className="text-center py-10 text-slate-500 border border-dashed border-slate-200 rounded-md">
 ...
 <td className="px-5 py-4">
 <div className="text-slate-900 font-bold">{payment.date}</div>
 ...
 <div className="border-t border-slate-200 bg-transparent p-2 text-center">
 <button
 ...
 const nextStatus = safeStatus === 'cancelled' ? null : statusFlow[currentIdx + 1];
 return (
 <div key={order.id} className="p-4 bg-transparent border border-slate-200 rounded-md space-y-3 hover:bg-[#f4f4f5] transition">
 <div className="flex items-start justify-between gap-3 flex-wrap">
 <div>
 ...
 {order.deliveryAddress && typeof order.deliveryAddress === 'object' && (
 <div className="bg-transparent border border-slate-200 rounded-md p-3 flex gap-3">
 <MapPin className="h-4 w-4 text-slate-500 shrink-0 mt-0.5" />
 <div>
 <p className="text-xs font-bold text-slate-900">{order.deliveryAddress?.name} ({order.deliveryAddress?.phone})</p>
 <p className="text-xs text-slate-500 mt-0.5">
