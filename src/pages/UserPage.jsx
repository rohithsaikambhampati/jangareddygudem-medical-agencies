...
const colorMap = {
  slate:   { ring: 'ring-slate-400',   bg: 'bg-slate-400',   text: 'text-slate-500',   light: 'bg-transparent',   border: 'border-slate-200'   },
        ...
        {order.statusHistory?.length > 0 && (
          <div className="mt-2">
            <button
              ...
              className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-500 font-medium transition"
            >
            ...
            <React.Fragment key={step.key}>
              <div className="flex flex-col items-center">
                <div className={`w-9 h-9 rounded-md flex items-center justify-center ring-2 transition-all duration-300 ${
                  isDone ? `${c.bg} ring-transparent shadow-sm` : 'bg-slate-50 ring-slate-200'
                }`}>
                  <Icon className={`h-4 w-4 ${isDone ? 'text-white' : 'text-slate-500'}`} />
                </div>
                <span className={`text-[10px] font-bold mt-1 ${isCurrent ? c.text : isDone ? 'text-slate-500' : 'text-slate-500'}`}>
                  {step.label}
                </span>
              </div>
...
function MyOrders({ userId }) {
  ...
  if (myOrders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
        <div className="w-20 h-20 bg-slate-50 rounded-md flex items-center justify-center">
  ...
  const statusBadge = (status) => {
    const map = {
      processing: 'bg-slate-50 text-slate-500 border-slate-200',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-medium px-3 py-1 bg-slate-50 text-slate-500 rounded-md">{myOrders.length} order{myOrders.length !== 1 ? 's' : ''}</span>
      </div>

      {myOrders.map((order) => (
        <div key={order.id} className="bg-white rounded-md border border-slate-200 shadow-sm p-4 space-y-4 hover:shadow-md transition">
          {/* Order header */}
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="font-bold text-slate-900 text-base leading-snug">{order.productName}</p>
...
function AddressModal({ currentUser, onConfirm, onCancel }) {
  ...
  const inputCls = 'w-full px-3.5 py-2.5 rounded-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition text-sm text-slate-900';
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      {/* Modal Content */}
      <div className="relative bg-white rounded-md shadow-sm w-full max-w-md overflow-hidden">
        ...
        {saved && usingSaved && (
          <div className="mx-6 mt-4 bg-teal-50 border border-teal-200 rounded-md px-4 py-3 flex items-center justify-between gap-3">
            <div>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3">
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onCancel}
              className="flex-1 py-2.5 rounded-md border border-slate-200 text-slate-500 font-medium text-sm transition">
              Cancel
            </button>
            <button type="submit"
              className="flex-1 py-2.5 rounded-md bg-teal-600 hover:bg-teal-700 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-sm transition">
    ...
    <div className="space-y-4 animate-fadeIn relative">
      ...
      {/* Toast */}
        {toast && (
          <div 
            key="toast"
            className="fixed top-20 right-6 z-50 flex items-center gap-3 bg-white text-slate-900 px-5 py-4 rounded-md shadow-sm border border-slate-800"
          >
            {toast.type === 'success'
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-40 bg-teal-600 hover:bg-teal-700 text-white p-4 rounded-md shadow-sm flex items-center justify-center transition-all duration-300 hover:scale-105"
      >
      ...
      <div className="bg-white border border-slate-200 px-5 py-3.5 rounded-md flex flex-wrap items-center justify-center sm:justify-start gap-4 shadow-sm">
        <span className="text-sm font-bold text-slate-900">Contact Owner:</span>
        <a href="tel:9440103869" className="flex items-center gap-2 text-sm font-semibold text-teal-600 hover:text-teal-800 transition bg-teal-50 px-3 py-1.5 rounded-md border border-teal-100">
          <Phone className="h-4 w-4" /> 9440103869
        </a>
        <a href="https://mail.google.com/mail/?view=cm&fs=1&to=himagiriprasadkambhampati@gmail.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm font-semibold text-teal-600 hover:text-teal-800 transition bg-teal-50 px-3 py-1.5 rounded-md border border-teal-100">
          <Mail className="h-4 w-4" /> himagiriprasadkambhampati@gmail.com
        </a>
      </div>

      {/* Tab Bar */}
      <div className="flex gap-1 bg-white border border-slate-200 shadow-sm p-1.5 rounded-md">
        <button
        ...
        <div className="space-y-4">
            ...
            <div className="flex justify-between items-center gap-4 flex-wrap">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition text-sm"
                />
              </div>
              <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                <select
                  ...
                  className="flex-1 sm:flex-initial py-2.5 px-4 rounded-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition text-sm text-slate-900 bg-white cursor-pointer"
                >
                  {uniqueBrands.map(b => <option className="bg-white text-slate-900" key={b} value={b}>{b === 'All' ? 'All Brands' : b}</option>)}
                </select>
                ...
                <select
                  ...
                  className="flex-1 sm:flex-initial py-2.5 px-4 rounded-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition text-sm text-slate-900 bg-white cursor-pointer"
                >
                  <option className="bg-white text-slate-900" value="name-asc">A to Z</option>
                  <option className="bg-white text-slate-900" value="name-desc">Z to A</option>
                  <option className="bg-white text-slate-900" value="price-asc">Price: Low to High</option>
                  <option className="bg-white text-slate-900" value="price-desc">Price: High to Low</option>
                </select>

                <label className="flex-1 sm:flex-initial flex items-center justify-center gap-2 cursor-pointer py-2.5 px-4 rounded-md border border-slate-200 hover:bg-transparent transition select-none bg-white">
                ...
                {totalCartItemsPaid > 0 && (
                  <button
                    onClick={() => setIsCartOpen(true)}
                    className="w-full sm:w-auto flex items-center justify-center gap-2 bg-teal-50 text-teal-700 hover:bg-teal-100 px-4 py-2.5 rounded-md text-sm font-bold transition shrink-0 sm:ml-auto"
                  >
            ...
            <div className="flex overflow-x-auto gap-2 pt-2 pb-1 scrollbar-none -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap">
              <button
                ...
                className={`px-4 py-1.5 rounded-md text-xs font-bold transition-colors ${
                  categoryFilter === 'All' ? 'bg-teal-600 text-white shadow-sm' : 'bg-white text-slate-500 border border-slate-200 hover:bg-transparent'
                }`}
              >
                All Categories
              </button>
              {categories.map(cat => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-md text-xs font-bold transition-colors ${
                    categoryFilter === cat ? 'bg-teal-600 text-white shadow-sm' : 'bg-white text-slate-500 border border-slate-200 hover:bg-transparent'
                  }`}
                >
                  {cat}
                </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredProducts.length === 0 ? (
              <div className="col-span-full text-center py-16 text-slate-500 bg-white rounded-md border border-dashed border-slate-200">
              ...
              filteredProducts.map((product) => {
                ...
                return (
                  <div key={product.id} className="bg-white rounded-md border border-slate-200 shadow-sm hover:shadow-md transition duration-200 overflow-hidden flex flex-col justify-between">
                    ...
                    <div className="px-5 py-2 flex-1">
                      <h3 className="text-lg font-bold text-slate-900 tracking-normal">{product.name}</h3>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide mt-0.5">{product.brand || 'Unbranded'}</p>
                      {hasOfferText && (
                        <div className="mt-1.5">
                          <span className="inline-flex items-center gap-1 bg-teal-50 text-teal-700 text-xs font-bold px-2.5 py-1 rounded-md border border-teal-100 uppercase">
                            Deal: {product.offerText}
                          </span>
                        </div>
                      )}
                    ...
                    <div className="p-5 pt-3">
                      {quantityInCart > 0 ? (
                        <div className="flex items-center justify-between bg-teal-50 border border-teal-100 rounded-md p-1">
                          <button onClick={() => handleUpdateQty(product.id, quantityInCart - 1)}
                            className="w-10 h-10 bg-white hover:bg-slate-50 rounded-md flex items-center justify-center text-teal-700 font-bold transition shadow-sm border border-slate-150">
                            <Minus className="h-4 w-4" />
                          </button>
                          <span className="text-base font-bold text-teal-900 w-10 text-center">{quantityInCart}</span>
                          <button onClick={() => handleUpdateQty(product.id, quantityInCart + 1)}
                            className="w-10 h-10 bg-white hover:bg-slate-50 rounded-md flex items-center justify-center text-teal-700 font-bold transition shadow-sm border border-slate-150">
                        ...
                        <button
                          ...
                          className="w-full font-medium py-2.5 px-4 rounded-md transition duration-200 shadow-sm flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-700 text-white"
                        >
                          Add to Cart
                        </button>
                      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white p-4 rounded-md border border-rose-100 shadow-sm flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 mt-1">₹{remainingOutstanding.toFixed(2)}</p>
            ...
            <div className="bg-white p-4 rounded-md border border-teal-100 shadow-sm flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 mt-1">₹{totalLifetimePaid.toFixed(2)}</p>
            ...
            <div className="bg-white p-4 rounded-md border border-emerald-100 shadow-sm flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 mt-1">₹{totalLifetimeDue.toFixed(2)}</p>
            ...
            {myPaymentsList.length === 0 ? (
              ...
              <div className="overflow-x-auto rounded-md border border-slate-200">
                <table className="min-w-full divide-y divide-slate-100 text-left text-sm">
                  <thead className="bg-transparent text-slate-500 font-medium text-xs uppercase tracking-wider">
                    <tr>
        ...
        <div className="fixed inset-0 z-50 overflow-hidden">
          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex pl-4 sm:pl-10">
            <div className="w-screen max-w-md bg-white shadow-sm flex flex-col">
              ...
              {/* Drawer Header */}
              <div className="p-4 border-b border-slate-200 flex items-center justify-between bg-transparent">
                <div className="flex items-center gap-2.5">
                  <ShoppingCart className="text-teal-600 h-6 w-6" />
                  <h2 className="text-xl font-bold text-slate-900">Verify Order</h2>
                </div>
                <button onClick={() => setIsCartOpen(false)} className="p-1 rounded-md hover:bg-slate-200 text-slate-500 hover:text-slate-500 transition">
                  <X className="h-6 w-6" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {cart.length === 0 ? (
                  <div className="text-center py-20 text-slate-500 flex flex-col items-center gap-4">
                    <ShoppingBag className="h-16 w-16 text-slate-900" />
                    <div>
                  ...
                  <div className="space-y-4">
                    {cart.map((item) => {
                      ...
                      return (
                        <div key={item.id} className="p-4 bg-transparent border border-slate-200 rounded-md flex flex-col gap-3 hover:bg-transparent transition">
                            ...
                            <div className="flex flex-col gap-1.5 bg-amber-50 border border-amber-200/50 p-2.5 rounded-md">
                              ...
                              {item.discountPercentage > 0 && (
                                <div className="text-xs font-bold text-teal-800 flex items-center gap-1.5">
                                  <span className="bg-teal-600 text-white font-bold px-1.5 py-0.5 rounded text-[10px]">SAVE</span>
                          ...
                          <div className="flex justify-between items-center mt-1 pt-1.5 border-t border-slate-200">
                            <div className="flex items-center gap-2">
                              <button onClick={() => handleUpdateQty(item.id, item.quantity - 1)}
                                className="w-7 h-7 bg-white hover:bg-slate-50 rounded-md border border-slate-200 flex items-center justify-center text-slate-500 transition">
              ...
              {/* Drawer Footer */}
              {cart.length > 0 && (
                <div className="p-4 border-t border-slate-200 bg-transparent space-y-4">
                  <div className="space-y-2 text-sm text-slate-500">
                    <div className="flex justify-between">
                      <span>Billed Units</span>
                      <span className="font-bold text-slate-900">{totalCartItemsPaid}</span>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-3.5 px-4 rounded-md transition shadow-sm flex items-center justify-center gap-2"
                  >
